...
public class DaoCloudConfig {
    ...
    public static <T> T getConf(ProxyConfigModel proxyConfigModel, Class<T> clazz) {
        ...
        if (!StringUtils.hasLength(jsonValue)) {
            // no hit cache
            DaoMessage daoMessage = new DaoMessage(Protocol.DEFAULT_VERSION, MessageType.PULL_REGISTRY_CONFIG_REQUEST_MESSAGE, DaoCloudConstant.DEFAULT_SERIALIZE, proxyConfigModel);
